...
A goal can be scored only if the ball is either kicked and \removed{clearly moves as
determined by the referee} \added{moves at least 5 cm} or has been touched by another player before being
...
\item indirect free kicks to the attacking team for an offence inside the
  opponents' \removed{goal}\added{penalty} area are taken from the nearest point
  on the \removed{goal}\added{penalty} area line which runs parallel to the goal
  line.
\removedSection{
\item \removed{free kicks to the defending team in their goal area may be taken from anywhere in that area}
}
\end{itemize}
...
\item is in play when it is kicked and \removed{clearly moves as determined by the
      referee} \added{moves at least 5 cm} except for a free kick to the defending team in their penalty area
...
The referee\removed{ blows the whistle,} announces 'Free-Kick' blue or
red\added{ and communicates ``Direct / Indirect Free Kick'' Blue/Red to the
  GameController.}%
\removed{%
  and then places the ball depending on the call.
  ...
  Robots which are significantly moving during this phase are removed from the
  field.%
}
The referee places the ball depending on the call and announces
``Free Kick Ready'' and \removed{the assistant referee clicks on }\added{communicates}
...
the free kick.
\added{Any player from} the team taking the free kick may announce that the
player is ready to take the free kick at any point \added{by sending a
message to the GameController}.
...
Once the free kick can be executed, the referee \removed{blows the whistle and the assistant
referee clicks on} \added{communicates} ``Execute Direct / Indirect Free Kick'' Blue/Red \added{to the GameController}.
...
\removedSection{
  ...
  \begin{itemize}
  \item \removed{the kick is retaken \simplify{(new:) }if the goal keeper
